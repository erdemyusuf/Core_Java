...
public class C02_ArraydenListOlusturma {
    ...
    public static void main(String[] args) {
        ...
        Random rnd=new Random();  //scanner gibi calisan class tır. bu class sayi uretir objedir atamayı asagıda while loop icinde yapacagız
        int sayi=0;
        List<Integer> sayiListesi=new ArrayList<>();

        while(sayiListesi.size()<200){
            sayi= rnd.nextInt(1000); // bound sınıra gore sayi uretir.
        ...
        // kullanicidan sayi alarak veya bulana kadar yapmak istemesi secimliktir. biz bulmayı sectik
        boolean bildiMi=false;
        int tahminSayisi=1;
        Scanner scan= new Scanner(System.in);


        while(!bildiMi){ // bildiMi==false ---> while loop true oldugu surece calısır
